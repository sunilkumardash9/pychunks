...
from pychunks.llama_index.types import MetadataAwareTextSplitter
from pychunks.llama_index.utils import split_by_char, split_by_sep
from pychunks.llama_index.helper import GlobalsHelper
...
class TokenTextSplitter(MetadataAwareTextSplitter):
    ...
    tokenizer: Callable = Field(
        default_factory=helper.tokenizer(),  # type: ignore
    ...
    def __init__(
        ...
        tokenizer = tokenizer or helper.tokenizer()
        super().__init__(
    ...
    def _split(self, text: str, chunk_size: int) -> List[str]:
        ...
        """
        #all_seps = [self.separator] + (self.backup_separators or [])
        #_split_fns = [split_by_sep(sep) for sep in all_seps] + [split_by_char()]
